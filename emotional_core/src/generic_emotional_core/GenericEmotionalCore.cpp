...
error_t GenericEmotionalCore::UpdateCurrentEmotionalState() {
...
error_t GenericEmotionalCore::WriteSensorData(SensorDataStruct_t data) {
...
const EmotionalStateDescriptorStruct_t *GenericEmotionalCore::GetState() {
...
error_t GenericEmotionalCore::WriteTempImpact(const TemporaryCoreImpact_t &data) {
...
error_t GenericEmotionalCore::UpdateCoreParamsFromSensorData(const SensorDataStruct_t &data) {
...
const InDataDescriptorStruct_t *GenericEmotionalCore::GetDescriptorForInData(const SensorDataStruct_t &data) const {
...
error_t GenericEmotionalCore::CheckParamVsCondition(const string &par_name, const float &par_val, const ConditionStruct_t &cond) {
...
bool GenericEmotionalCore::CheckState(const EmotionalStateDescriptorStruct_t *emo) {
    bool res = false;
    error_t comp_result;
...
error_t GenericEmotionalCore::UpdateCoreParamsWithTimeUpdate(const int &time_duration_ms) {
    float change;
...
error_t GenericEmotionalCore::WriteTime(const int &time_duration_ms) {
...
error_t GenericEmotionalCore::SetState(const EmotionalStateDescriptorStruct_t *state) {
    _emotionalState_p = state;
    return NO_ERROR;
}

GenericEmotionalCore::GenericEmotionalCore() : _emotionalState_p(nullptr),
                                               _non_specified_state({.name = "non-specified", .conditions = {}}) {}

error_t GenericEmotionalCore::UpdateParamsTotal() {
    float val;
...
error_t GenericEmotionalCore::AddParam(const string &name, float default_value) {
...
error_t GenericEmotionalCore::AddState(EmotionalStateDescriptorStruct_t newState) {
    for (auto i = newState.conditions.begin(); i != newState.conditions.end(); i++) {
        if (!_params_base.IsParamExists(i->param)) {
            CRITICAL_ERROR(ERROR_NOENT, "Parameter in the Emotional State Descriptor is not represented in this core yet.");
        }
    }
...
error_t GenericEmotionalCore::AddSensorDataDescriptor(const InDataDescriptorStruct_t &newDescriptor) {
