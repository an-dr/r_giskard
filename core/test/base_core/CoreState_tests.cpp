...
void SystemState_test() {
    ...
    int res;
    res = st.core_params.AddParam("curiosity");
    printf("%d\n", res);

    res = st.core_params.AddParam("comfort", 10.1);
    printf("%d\n", res);

    res = st.core_params.SetParam("not a param", 100);
    printf("%d\n", res);

    res = st.core_params.SetParam("curiosity", 100.500);
    printf("%d\n", res);


    printf("%f\n", st.core_params.GetParam("curiosity"));

    res = st.SetState(es.Get("panic"));
    printf("%d\n", res);

    const emotional_state_t *current_state = st.GetState();
